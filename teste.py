...
def atualizar_lancamento_receitas_aluguel():
    # Limpa o conteúdo atual do frame
    for widget in frame_conteudo.winfo_children():
        widget.destroy()
    
    # Cabeçalho informando o prédio selecionado
    label_predio = ctk.CTkLabel(frame_conteudo, text=f"Prédio Selecionado: {prédio_selecionado}", font=("Arial", 14, "bold"))
    label_predio.pack(pady=10)
    
    # Título do formulário
    ctk.CTkLabel(frame_conteudo, text="Lançar Receita - Aluguel", font=("Arial", 18, "bold")).pack(pady=10)
    
    # Carregar os inquilinos do arquivo Excel
    inquilinos = carregar_inquilinos()  # Função que já retorna uma lista de dicionários
    ...
    # Campo somente para exibir o valor total (aluguel + IPTU + condomínio)
    ctk.CTkLabel(frame_conteudo, text="Valor Total (Aluguel + IPTU + Condomínio):").pack(pady=5)
    campo_valor_total = ctk.CTkEntry(frame_conteudo, state="disabled", width=200)
    campo_valor_total.pack(pady=5)
    
    # Atualiza o campo de valor total quando um inquilino for selecionado
    def atualizar_valor_total(event):
        inquilino_nome = combo_inquilino.get()
        inq = next((i for i in inquilinos if i['nome'] == inquilino_nome), None)
        if inq:
            # Somando os valores (assegure-se de que os valores estão salvos como números no Excel)
            total = inq['valor_aluguel'] + inq['valor_iptu'] + inq['valor_condominio']
            campo_valor_total.configure(state="normal")
            campo_valor_total.delete(0, "end")
            campo_valor_total.insert(0, f"{total:.2f}")
            campo_valor_total.configure(state="disabled")
    
    combo_inquilino.bind("<<ComboboxSelected>>", atualizar_valor_total)
    
    # Label para status do lançamento
...
def atualizar_lancamento(tipo):
    ...
    if tipo == "lancar_despesas":
        ...
        ctk.CTkLabel(frame_conteudo, text="Categoria:").pack(pady=5)
        # Remova a opção "Aluguel" da lista de categorias de receitas
        categorias_receitas_filtradas = [cat for cat in categorias_receitas if cat.lower() != "aluguel"]
        combo_categoria_receita = ctk.CTkComboBox(frame_conteudo, values=categorias_receitas_filtradas)
        combo_categoria_receita.pack(pady=5)
        
        # Caso seja necessário selecionar outro dado (por exemplo, se o lançamento for dividido entre prédios)
        ...
        # Campos de porcentagem para divisão
        frame_porcentagem = ctk.CTkFrame(frame_conteudo)
        ctk.CTkLabel(frame_porcentagem, text="GV (%):").pack(side="left", padx=5)
        campo_porcentagem_gv = ctk.CTkEntry(frame_porcentagem, width=50)
        campo_porcentagem_gv.pack(side="left", padx=5)
        ...
        campo_porcentagem_jlp.pack(side="left", padx=5)
        frame_porcentagem.pack(pady=5)
        frame_porcentagem.pack_forget()  # Esconde inicialmente

        # Função para mostrar/ocultar os campos de porcentagem
        def toggle_campos_porcentagem():
        ...
        # Função para salvar a receita (não inclui o lançamento de aluguel)
        def salvar_receita():
            ...
            try:
                ...
                label_status_lancamento.configure(text="Insira um valor numérico válido.", text_color="red")
                return
            ...
            if dividir_receita_var.get():
                try:
                    ...
                    salvar_lancamento_em_excel("Receita", valor_gv, categoria, "GV", "", 0, mes_usuario)
                    salvar_lancamento_em_excel("Receita", valor_jlp, categoria, "JLP", "", 0, mes_usuario)
                ...
                saldo_inicial = 0  # Ajuste se necessário
                salvar_lancamento_em_excel("Receita", valor, categoria, prédio_selecionado, "", saldo_inicial, mes_usuario)
                label_status_lancamento.configure(
